...
L.ComputeDist = L.Toolbar2.Action.extend({
    options: {
    ...
    _onMove: function(event) {
        ...
        if (this._startPath){
            if(this._startMove )
            {
                this._currentPath.setLatLngs(this._currentDist._latlngs.concat(latlng)) 
            }
            else
            {
               this._currentPath.addLatLng(latlng);
    ...
    _finishCompute: function(event) {
        if (this._startPath) {
            if(this._currentDist._latlngs.length > 1)
            {
                if(!event || event.type === "contextmenu" || event.type === "dblclick") {
                    this._currentPath.setLatLngs(this._currentDist._latlngs);
                    this._addReomveMarker(this._lastPoint);
                    this._currentDist.lastLable._container.style.display = "none";
                    this._addLable(this._lastPoint, this._getDistanceString(this._totalDist), "dist-end");
                }
            } 
            else
            {
                this._clearLayers.call(this._currentDist);
            }
        }
...
L.ComputeArea = L.Toolbar2.Action.extend({
    options: {
            toolbarIcon: { className: 'leaflet-ComputeDist' }
        },
        _rad: Math.PI / 180,
        ...
        _setArea: function(event) {
            ...
            if(this._startArea)
            {
                ...
                {
                    this._addMarker(latlng);
                    this._compute._latlngs.push(latlng);
                }
            }
            else{
                this._compute = {
                ...
                this._addArea(latlng);
                this._map.on("mousemove", this._onMove, this);
                this._startArea = true;
                this._addMarker(latlng);
                this._compute._latlngs.push(latlng);
            }
        ...
        _onMove: function(event) {
            ...
            {
                if(this._startMove ){
                    this._area.setLatLngs(this._compute._latlngs.concat(latlng));
                } else{
                    this._area.addLatLng(latlng);
                    this._startMove = true;
                }
            }
        },
        _finishCompute: function(event) {
            //this._startArea && (this._compute._latlngs.length > 2 ? 
            //((!event || event.type === "contextmenu") && this._area.setLatLngs(this._compute._latlngs), this._addReomveMarker(this._lastPoint), this._addLable(this._lastPoint, this._getAreaString(this._compute._latlngs), "area-result")) 
            //: this._clearLayers.call(this._compute));
            if(this._startArea)
            {
                if(this._compute._latlngs.length > 2)
                {
                    if(!event || event.type === "contextmenu" || event.type === "dblclick" ) {
                        this._area.setLatLngs(this._compute._latlngs);
                        this._addReomveMarker(this._lastPoint);
                        this._addLable(this._lastPoint, this._getAreaString(this._compute._latlngs), "area-result");
                    }
                }
                else{
                    this._clearLayers.call(this._compute)
